...
import { ArrowRight, ArrowLeft, CheckCircle2, Shield, Zap, Clock, Download } from 'lucide-react';
  ...
  const handleDownload = useCallback(() => {
    ...
    const printContent = `
      ...
      <body>
        ...
        ${formData.email ? `<div class="field"><span class="label">Email</span><span class="value">${formData.email}</span></div>` : ''}
        ${formData.phone ? `<div class="field"><span class="label">Phone</span><span class="value">${formData.phone}</span></div>` : ''}
        ...
        ${formData.drivingLicenseNumber ? `<div class="field"><span class="label">${t('drivingLicenseNumber')}</span><span class="value">${formData.drivingLicenseNumber}</span></div>` : ''}
        ${formData.marks10th ? `<div class="field"><span class="label">10th Marks</span><span class="value">${formData.marks10th}</span></div>` : ''}
        ${formData.marks12th ? `<div class="field"><span class="label">12th Marks</span><span class="value">${formData.marks12th}</span></div>` : ''}
        ${formData.sgpa ? `<div class="field"><span class="label">Engineering SGPA</span><span class="value">${formData.sgpa}</span></div>` : ''}
        ${formData.codechefLink ? `<div class="field"><span class="label">CodeChef</span><span class="value"><a href="${formData.codechefLink}">${formData.codechefLink}</a></span></div>` : ''}
        ${formData.leetcodeLink ? `<div class="field"><span class="label">LeetCode</span><span class="value"><a href="${formData.leetcodeLink}">${formData.leetcodeLink}</a></span></div>` : ''}
        ${formData.githubLink ? `<div class="field"><span class="label">GitHub</span><span class="value"><a href="${formData.githubLink}">${formData.githubLink}</a></span></div>` : ''}
        ${formData.gfgLink ? `<div class="field"><span class="label">GeeksforGeeks</span><span class="value"><a href="${formData.gfgLink}">${formData.gfgLink}</a></span></div>` : ''}
        ${formData.hackerrankLink ? `<div class="field"><span class="label">HackerRank</span><span class="value"><a href="${formData.hackerrankLink}">${formData.hackerrankLink}</a></span></div>` : ''}
        ${formData.linkedinLink ? `<div class="field"><span class="label">LinkedIn</span><span class="value"><a href="${formData.linkedinLink}">${formData.linkedinLink}</a></span></div>` : ''}
        <div class="footer">
    ...
    fetch(`${basePath}docfill-extension.zip`)
      ...
      .then((blob) => {
        ...
        toast({
          ...
          description: 'Unzip and load it in chrome://extensions (Developer mode → Load unpacked).',
        });
      })
      .catch((err) => {
        toast({
          title: 'Download failed',
          description: err instanceof Error ? err.message : 'Could not download extension',
          variant: 'destructive',
        });
      });
  }, [toast]);
          ...
          <section className="text-center mb-12 animate-fade-in">
            ...
            <div className="mt-8 flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-muted-foreground">
                Always grabs the latest version. Reload it in chrome://extensions after downloading.
              </p>
            </div>
          </section>
        )}
